...
swim_pipeline.py
────────────────
Combined BLE recording + signal processing pipeline for AS5600 swim encoder data.

Usage:
    python swim_pipeline.py record [--name DEVICE_NAME]
        Scan for BLE device, prompt for a session label, record to raw/, then
        automatically process the file when you press Ctrl+C.

    python swim_pipeline.py process [INPUT] [--fs HZ]
        Process an existing raw CSV or a directory of CSVs.
        INPUT defaults to raw/{SESSION_STEM}.csv if omitted.

Edit the constants below before each session as needed.
"""
...
PACKET_SIZE     = 14        # 2 samples × 7 bytes
SAMPLE_SIZE     = 7
SAMPLES_PER_PKT = PACKET_SIZE // SAMPLE_SIZE

# ── Default session for `process` subcommand ──────────────────────────────────
SESSION_STEM = "kenneth_3"

# ── Excluded time ranges ──────────────────────────────────────────────────────
# List of (start_s, end_s) pairs to NaN out (e.g. rest between sets).
EXCLUDED_SEGMENTS = [
...
# ── Decimation settings ────────────────────────────────────────────────────────
TARGET_FS_HZ = 100.0   # output rate after decimation (~5× from ~270 Hz native)
...
def parse_packet(data: bytes) -> list[tuple]:
    ...
    for i in range(SAMPLES_PER_PKT):
        ...
        ts, angle, mag = struct.unpack_from("<IHB", data, offset)
        samples.append((ts, angle, mag))
    return samples


async def run(device_name: str, out_path: Path) -> None:
    print(f"Scanning for '{device_name}'...")
    device = await BleakScanner.find_device_by_name(device_name, timeout=15)
    if device is None:
        print(f"ERROR: '{device_name}' not found. Is the Nano powered on and advertising?")
        return
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Swim encoder pipeline: record via BLE and/or process raw CSVs.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
examples:
  python swim_pipeline.py record
  python swim_pipeline.py record --name MyDevice
  python swim_pipeline.py process raw/swim_tony_20250515_103000.csv
  python swim_pipeline.py process raw/
  python swim_pipeline.py process raw/swim_tony_20250515_103000.csv --fs 50
""",
    )
    sub = parser.add_subparsers(dest="cmd", required=True)

    rec_p = sub.add_parser("record", help="Record a BLE session, then auto-process on Ctrl+C")
    rec_p.add_argument("--name", default=DEVICE_NAME,
                       help=f"BLE device name to connect to (default: {DEVICE_NAME})")

    proc_p = sub.add_parser("process", help="Process an existing raw CSV or directory of CSVs")
    proc_p.add_argument("input", nargs="?", default=f"raw/{SESSION_STEM}.csv",
                        help="CSV file or folder of CSV files (default: raw/%(default)s)")
    proc_p.add_argument("--fs", type=float, default=TARGET_FS_HZ,
    ...
    if args.cmd == "record":
        ...
        try:
            asyncio.run(run(args.name, out_path))
        except KeyboardInterrupt:
            pass

        Path("processed").mkdir(parents=True, exist_ok=True)
        output_file = Path("processed") / f"{out_path.stem}.csv"
        print(f"\n{'=' * 50}")
        print(f"Auto-processing: {out_path}  ->  {output_file}")
        print(f"{'=' * 50}")
        process_file(out_path, output_file, TARGET_FS_HZ)
