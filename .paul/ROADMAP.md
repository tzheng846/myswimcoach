...
| 67 | External Camera Sync (GoPro / external waterproof cam) | **✅ COMPLETE (2/2 plans) 2026-08-17.** 67-01 one-tap push-off align on the web annotate page (`2aa58ca` → Vercel); 67-02 memory-safe uploads — 413 before buffering + streamed to Storage instead of `await file.read()` — with a free-tier 50 MB cap + compress/upgrade messaging (`030f6f9`, `e3ce464`). ⚠ **Mid-phase pivot:** user is on the Supabase FREE tier (hard 50 MB ceiling), so the "raise the cap to 500 MB" premise was invalidated — pivoted (user-chosen via AskUserQuestion) to guide manual compression + defer real >50 MB footage to a Pro upgrade (a documented one-flip: raise global limit + apply `patch_11` + bump `MAX_VIDEO_BYTES`), NOT throwaway in-browser transcoding. V1 = one clip per session; the long-take → many-sessions shared-asset workflow is a deferred phase. Suite 58 green; zero regressions. ⚠ Real-clip UAT (align feel + <50 MB upload) owed. Discussed 2026-08-16 via `/paul:discuss` (2 structural forks via AskUserQuestion); CONTEXT.md written, decisions D1–D6 (+D2b). **WEB-FIRST, near-zero backend.** TRIGGER: today the only video is the phone's in-app camera; coaches want to shoot with a GoPro/waterproof cam — also the only way to capture the underwater pulldown (synergy with Phase 65). Two field workflows named: **(uc2)** start/stop per session = one clip : one session; **(uc1)** one long take → many sessions, some scrapped between. ⭐ **REPO-VERIFIED, and it SHRINKS the phase:** `VideoPane` ALREADY has web upload (`attach()`, `:260`), a manual ±0.1 s nudge (`:225`), Save-sync (`:242`) and the 58-04 end-anchor auto-origin (`:86`,`:204`) — the web can already attach + nudge. What's MISSING is the one-tap **push-off visual align**, and the fact that the end-anchor (`sessionDuration − videoDuration`, 44-03) is WRONG for an external clip (video + session don't stop together) → dozens of nudges. DECISIONS: **D1 V1 = uc2 only** — reuse the 1:1 `video_path`/`video_origin_s`; the uc1 shared-asset model `(video_asset_id, offset_s)` is a DEFERRED follow-up (ordinal "Nth swim = Nth session" is a trap once a session is scrapped — each must anchor independently); **D2 primary anchor = visual push-off align done BY THE COACH, NOT computer vision** (user distrusts CV underwater — correctly); **D2b only the VIDEO side needs a human** — the dive/push-off session-time is the encoder's velocity spike, auto-detected by `build_seed` (`dive_start_s = baseline_end_s`, `annotations.py:114`), so the coach scrubs to the push-off frame and one tap sets `origin = diveSessionTime − videoTime`; **D3** clock auto-match = deferred HINT only (no persisted true swim-start — `recorded_at` is upload time; GoPro clocks lie); **D4** upload surface = web/desktop (iOS in-app path untouched); **D5** `video_origin_s` MEANING unchanged, only its DERIVATION (phone = end-anchor, external = align — same column, same reader); **D6** reuse `POST /video` as-is. ⚠ **REAL ROBUSTNESS GATES, the honest "how robust is it" answer (→ 67-02):** `POST /video` does `await file.read()` (whole file into RAM — OOM on a big clip) and the `videos` bucket was created with **no `file_size_limit`** (`patch_07:59-61`) so it inherits Supabase's ~50 MB default — real GoPro footage is rejected/OOM TODAY. **2-plan phase:** **67-01** push-off align (`web/`: `VideoPane` + annotate wiring; `autonomous:true`, 2 tasks, verify with a <50 MB clip) → **67-02** production-size robustness (Supabase bucket cap raise [human-action] + memory-safe backend size guard + client format/size handling). Context: `.paul/phases/67-external-camera-sync/CONTEXT.md` |

| 68 | Persist Generated Session Names (TODO, unscheduled) | Raised 2026-08-16 while measuring Phase 65's reported "indigo ray" session — it could NOT be found by name because Phase 61-05 derives the creative mnemonic at RENDER time (`web/lib/sessionName.js` `displayName`, from the session id) and never writes `sessions.name`, so a session is only reachable by uuid (which is why `underwater_probe.py` needed a `--id` flag). User's call: **store the generated name into `sessions.name` on session creation so it is stable + queryable, UNLESS the user overwrites it.** Likely touches: the `/process` insert (`api.py`) to compute + store a name when none is supplied, and a backfill for existing NULL-name rows; the web/mobile `displayName` fallback then becomes a safety net rather than the source of truth. ⚠ Reverses Phase 61-05's "derived, never written" decision — confirm the mnemonic algorithm is deterministic + collision-tolerant before persisting, and decide whether a rename should regenerate. Not planned. |
| 69 | Multi-Camera Video (up to 4 synced angles) | **✅ CODE COMPLETE (3/3 plans) 2026-08-17 — shipped; ⚠ patch_12 + UAT owed.** Discussed 2026-08-17 (mockup + 2 forks via AskUserQuestion); CONTEXT D1–D8. Up to 4 camera angles per session (phone + 3 external) on a dedicated `/app/sessions/[id]/videos` page with a synced one-timeline player. **69-01** `session_videos` table (externals-only, **ADDITIVE** — legacy columns + 5 web readers + mobile untouched, no migration) + external-video API (GET/POST/PATCH/DELETE `/videos`, unified list, 50 MB guard, cap 3), patch_12, suite 58→61 (`ca73421`). **69-02** the Videos page + `CameraTile` (attach/label/sync/delete, adaptive grid, per-camera push-off sync) (`57d06c9`). **69-03** `MultiCamPlayer` — one master timeline drives all cameras + the trace, focused-plays perf model (CONTEXT D6) — + report-card declutter (VideoTracePanel → compact Videos link) (`f03c4fd`). ⚠ **REFINED at plan time:** additive/externals-only instead of migrating the 1:1 columns, because 5 web sites read them directly. ⚠ **patch_12 NOT applied live** — user runs it in the SQL editor; nothing works until then (then re-run `tools/introspect_schema.py`). ⚠ **UAT owed**, esp. 4-video synced playback (highest-risk, built blind — no live data/videos/auth in the sandbox). ⚠ Free-tier 50 MB/clip cap (67-02) applies; long-take → many-sessions still deferred. Context: `.paul/phases/69-multi-camera-video/CONTEXT.md` |

| 74 | BLE Dump Reliability (fix "end of dump not received" + data loss) | Planning (74-01 created 2026-08-19). TRIGGER: at a pool test the iOS app repeatedly showed **"The end-of-dump marker never arrived"** at random, and the affected buffered session became **unretrievable** (user: *"the session data are not retrievable; we just kept trying new sessions until it worked"*) — the device stayed responsive, so the firmware was NOT frozen. ROOT CAUSES (code-verified): **(1) DATA LOSS** — `dumpBuffer()` (`ESP_32_V5.ino:494-496`) clears `bufCount=0; dataReady=false` unconditionally after `sendEndOfDumpMarker()`, so a dump the firmware *thinks* completed wipes the buffer even when the phone never received the marker/tail; the only retention path is a mid-dump disconnect (`:474-480`). **(2) SINGLE-POINT-OF-FAILURE STALL** — the whole retrieval hinges on one 1-byte `0xEE` indication arriving; the phone has no expected-count and no retry, so any lost marker → a 30 s stall (`RecordScreen.js:359-363`) → the generic error. The indication design (`notify(false)` "paces the loop, no drops", `:485-487`) drops or skips on congestion/lost-confirm with no recovery. **(3)** `#define TRACE_BUFFER 1` (`:86`) — the diagnostic build ("SET BACK TO 0 FOR THE NORMAL BUILD") is what's flashed. **(4)** no `requestMTU` on mobile (latent Android failure; iOS relies on the OS negotiating ≥171 for the 168-byte packets). **(5)** `log()` is `console.log` only — invisible in TestFlight, so pool failures are un-forensic. FIX DIRECTION: firmware retains the buffer until an explicit phone CLEAR (or next-recording overwrite) so every failed retrieval is retryable = **zero data loss**; resend the `0xEE` marker; TRACE_BUFFER→0; phone shortens the stall + auto-retries DUMP + surfaces received-count. ⚠ Firmware/mobile changes are hardware-verify-gated (flash + pool re-test). `swimnetics-mobile` is a separate user-owned repo. Context: `.paul/phases/74-ble-dump-reliability/CONTEXT.md`
| 75 | Report Card Revamp (Race-Phase Model: Start / Underwater / Swim) | **75-01 ✅ COMPLETE 2026-08-19 (Step 1/3) · 75-02 PLANNED 2026-08-19 (Step 2/3).** CONTEXT (17 decisions) resequenced this **backend-first into 3 ordered steps** (D11/D14, must survive session boundaries): (1) skeleton, (2) metrics one-by-one at explicit approval, (3) the phase-organized UI last. **75-01** shipped `phase_metrics.py` — a 37-spec `MetricSpec` REGISTRY covering the whole taxonomy, all `status="planned"`/`compute=None`, plus `PhaseContext`, `compute_phases()`, an additive `phases` object in `/process`, and **`POST /sessions/{id}/recompute`** as the recompute-from-stored-profiles backfill seam (2nd instance of that pattern after `PUT /annotations`). `1ba589a`; suite 282→317. **Zero metrics implemented, by design (D12).** **75-02** (`autonomous:false`, `depends_on ["75-01"]`, wave 1; 10 files incl. 2 new tools; 3 auto + 1 `checkpoint:human-verify`; 6 ACs) = the **underwater phase's START boundary + the 4 window-arithmetic underwater metrics** (`uw_duration` / `uw_distance` / `uw_avg_speed` / `uw_surface_ratio`, plus `pulldown_*` passthrough for breaststroke only). ⭐ **THE USER'S OWN HYPOTHESIS WAS MEASURED AND IT WINS BY AN ORDER OF MAGNITUDE.** User: *"for every session i've seen, the underwater phase begins at the first big velocity dip."* Coded as surge-peak → first velocity trough with prominence ≥ **0.40 × v95** and scored against the 38 coach annotations: **mean |err| 0.13 s, 35/38 within 0.5 s, 37/38 within 1.0 s, 0 missed** — vs the incumbent dive-peak rule (`baseline_end_s + dive_duration_s`) at **1.23 s on only 10 of 38**, null on **84 of 108** sessions and ~1.5 s systematically early (it marks the top of the dive, not the end of the glide). Flat 0.30–0.40 plateau (not a knife-edge fit); uniform across strokes (free 0.15 / fly 0.10 / breast 0.03). ⚠ CIRCULARITY CAVEAT: the coach placed those marks while looking at the velocity trace, so the rule may partly describe how the mark is clicked — but it is the annotation contract's own definition of the boundary, so it is the correct target for an auto seed. ⚠ **THE BREAKOUT / END BOUNDARY IS DEFERRED BY USER DIRECTIVE** (*"assume whatever the input for end boundary is correct"*) — and **three of three levers are now measured and failing**, recorded so no later plan re-tests them: mean-|vel| step REFUTED (65-01), accel surge dive-confounded (65-01), and a **rhythm step-down detector tested 2026-08-19 was WORSE than the incumbent** (17/37 no-detect, mean |err| 2.95 s). Current auto `stroke_start` is ~3–4 s off in both directions; the freestyle auto window is a median **0.42 s** wide (18/32 under 0.5 s). Same open problem as TODO #69 / Phase 58's removed breakout marker — a candidate phase of its own. CONSEQUENCE ACCEPTED (P3): underwater metrics blank on most unannotated freestyle rather than showing a wrong number. DECISIONS at plan time: P1 window = `[underwater_start_s, stroke_start_s]` (**resolves 65-A1 / CONTEXT open-call-3**); P2 precedence **coach annotation → auto**, per-key source recorded, and for `underwater_start` the auto is the NEW detector, never the dive-peak seed; P3 no trust gate — blank only on arithmetic impossibility; **P4 the dip rule replaces the dive-peak EVERYWHERE incl. `annotations.build_seed`**, so annotating a session drops from 4 marks to 1 (the practical unlock); P5 no `udk` special-casing (user); **P6 Claude's call — kick metrics split out to 75-03** (the user approved "kick detector + all 7 kick metrics" as the PHASE scope; splitting keeps 75-02 at PAUL sizing and gives the kick detector its own eyeball loop; flag at UNIFY if unwanted); P7 `pulldown_*` breaststroke-only. Validation choice (user): **synthetic unit tests + user eyeball on real traces**, NOT a video-derived kick ground truth. **75-03 (named, not written)** = the kick detector + `kick_count` / `dist_per_kick` / `kick_tempo` / `kick_consistency` / `per_kick_decay` / `first_kick_impulse` / `uw_ivv`. **Step 3** = the phase-organized report card under the D7 alerting doctrine (within-athlete contrast, **no absolute thresholds**), web first then iOS. ⚠ **65-03 was never executed and is ABSORBED here.** Context: `.paul/phases/75-report-card-phase-model/CONTEXT.md` |
| 77 | Fly Breakout Detection (arm-cycle appearance, butterfly) | **77-01 APPLIED 2026-08-20.** The butterfly counterpart of Phase 76, and a separate phase because the signal is inverted: 76 finds the free/back breakout where the ~2 Hz dolphin-kick band DISAPPEARS, but butterfly is a dolphin kick with arms bolted on — its surface stroke IS two dolphin kicks per arm cycle, so the band never switches off and the 76 rule measured WORSE than the incumbent on fly (4.46 s vs 2.43 s). Fly is an APPEARANCE problem: a ~1 Hz arm cycle turns on underneath the persistent ~2 Hz kick. Shipped `metrics.detect_breakout_fly` — the band-power ratio `P(0.8-1.1 Hz) / P(1.1-1.5 Hz)` detected as a rise AFTER a sustained low (the low-run requirement is what skips the push-off transient) — behind a butterfly-only branch at the same `compute_session_metrics` seam 76 uses, with the shared `_breakout_leaves_swim` collapse guard and the refuse-to-answer convention. MEASURED on the 16 annotated fly `stroke_start_s` marks: median |err| **2.67 s → 0.38 s, 12/16 within 1 s**, 1 refusal. Robustness measured, not assumed: a 4x4 band-edge jitter grid tops out at 0.94 s (every cell beats the incumbent → physical, not a knife-edge fit), and swapping the annotated underwater start for the auto `detect_underwater_start` (75-02) gives an IDENTICAL 0.38 s, so the production seam holds on fly. Two things were measured and REJECTED rather than shipped on faith: the per-session f0 band refinement (D5) scored 2.33 s vs 0.38 s for fixed physical bands and is left off, and `_FLY_MIN_CONTRAST` — a gate added during APPLY because the low/rise thresholds are drawn from the ratio's OWN range, so a merely rippling ratio could never be refused — sits at 1.5, the top of a flat 1.0-1.5 plateau, because >=2.0 converts good detections into incumbent fallbacks (0.38 → 0.63 s). No learned model (D2): 16 sessions from ONE swimmer is where 59-05's no-overfit rule bites hardest. Free/back/breaststroke/None are byte-identical (verified by rebuilding the pre-77 module and diffing every returned key). ⚠ COMPARABILITY BREAK for stored butterfly sessions + the backfill is a separate post-approval step (D6). ⚠ 75-03's fly kick metrics inherit this `stroke_start` (D7). Context: `.paul/phases/77-fly-breakout-detection/CONTEXT.md` |
| 69 | Freestyle/backstroke breakout placement — Mode-A residual (TODO, unscheduled) | Raised 2026-08-17 during 65-02's human-verify: on NON-railed free/back sessions `detect_swim_window`'s `ip_end` lands ~1–2 s EARLY vs the coach's eye and the one annotated case (`d25c578f`: detector 5.2 s vs human 6.48 s; user's eyeball ~5.7 s — right direction, less early than truth). This is the pre-existing Phase 59-03 settle residual (Mode A late / Mode B fallback-early), **NOT** the 65-02 rail (which is fixed). ⚠ **USER ASKED whether ACCELERATION would help — evaluated on 5 freestyle sessions (accel overlay) and REJECTED:** acceleration is exactly `dv/dt` (**no information independent of velocity** — the user's own "circular" instinct is correct), it is **just as large during the underwater kicks as during stroking** (the overlay visually confirms 65-01's dive-confounded/inconclusive accel finding), and differentiation is a **high-pass that AMPLIFIES the fast kicks** — the wrong direction for a stroke-vs-kick frequency detector. GATE: **start with GROUND TRUTH** (annotate a batch, ideally camera-synced via Phase 67) and MEASURE the real error BEFORE any code — n is 1 swimmer, 4 annotated freestyle, 0 backstroke. If code changes, the promising lever is a **FREQUENCY STEP-DOWN** rule (underwater kicks run ~2× stroke rate; mark the breakout where frequency drops to the stroke fundamental), NOT acceleration. Not planned. |
